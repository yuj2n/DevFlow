...
유진님, 맞습니다! "목록을 거치면 괜찮지 않나?"라는 의문에서 시작해 "리액트의 컴포넌트 재사용 메커니즘"까지 파고든 과정은 교수님이 보시기에 아주 훌륭한 학습 데이터입니다.

이 내용을 `docs/ai-logs/WEEK08_AI_LOG.md`에 추가할 수 있도록 일지 형식으로 정리해 드릴게요. "단순히 코드를 짠 게 아니라, 원리를 이해하고 방어적으로 설계했다"는 점을 강조했습니다.

---

### 🛠️ [LOG-7] 리액트 컴포넌트 재사용(Reconciliation) 특성 파악 및 방어적 설계

- **프롬프트 (질문):** "목록 페이지를 거쳐서 문서로 진입하면 컴포넌트가 새로 마운트되는데, 왜 굳이 에디터에 `key={id}`를 넣어야 해? 어차피 초기 데이터는 고정된 값 아냐?"
- **AI 제공 해결책:**
- 리액트는 성능 최적화를 위해 동일한 계층의 동일한 컴포넌트는 파괴하지 않고 재사용(Reconciliation)하려는 성질이 있음을 설명.
- 브라우저의 뒤로 가기/앞으로 가기, 혹은 향후 추가될 사이드바 직접 이동 시 발생할 수 있는 **'데이터 불일치(Data Desync)'** 위험성 경고.
- `key` 프롭을 통한 선언적 리마운트가 가장 안전한 **방어적 프로그래밍**임을 가이드.

- **새로 알게 된 점 (Learned):**
- **컴포넌트의 고집:** `useEditor` 같은 외부 라이브러리 훅은 리액트의 일반적인 상태(State) 변화를 자동으로 추적하지 않고 초기화 시점의 값에 의존하는 경우가 많음.
- **인스턴스 생명주기 제어:** `key` 프롭은 단순히 리스트 렌더링뿐만 아니라, 컴포넌트의 인스턴스를 강제로 교체하여 메모리를 비워주는 **'하드 리셋'** 도구로 사용할 수 있음.

- **직접 수행한 작업 (Human Action):**
- 현재의 네비게이션 구조(목록 -> 상세)에만 의존하지 않고, 어떤 경로로 문서가 전환되더라도 데이터 섞임 현상이 발생하지 않도록 `EditorPage` 내 `TiptapEditor`에 `key={id}` 속성 적용 완료.
- 이를 통해 앱의 확장성을 고려한 견고한 에디터 구조 확립.
